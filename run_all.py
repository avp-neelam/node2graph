...
def main():
    ...
    p.add_argument('--fusion', type=str, default='concat,attn')
    ...
    for ds, kset_str, model, fusion in itertools.product(datasets, args.ksets, models, fusions):
        ...
        combos.append((ds, ks, model, fusion))

    results = {}
    for (ds, ks, model, fusion) in combos:
        key = (ds, tuple(ks), model, fusion)
        vals = []
        for sd in seeds:
            _, test_acc = run_experiment(
                ...
                seed=sd, add_pos_feats=False, class_balance=not args.no_class_balance, cache=not args.no_cache,
                csv_log_path=args.csv
            )
            vals.append(test_acc)
        mu = statistics.mean(vals); sigma = statistics.pstdev(vals)
        results[key] = (mu, sigma)
        print(f"SUMMARY {ds} ks={ks} model={model} fusion={fusion} → Test {mu:.4f} ± {sigma:.4f} over {len(seeds)} seeds")

    summaries_path = os.path.join(os.path.dirname(args.csv), "new_summaries.csv")
    ...
    with open(summaries_path, "a", newline="") as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(["dataset", "ks", "model", "fusion", "num_seeds", "mean_test_acc", "std_test_acc"])
        for (ds, ks, model, fusion), (mu, sigma) in results.items():
            writer.writerow([
                ds,
                "-".join(map(str, ks)),
                model,
